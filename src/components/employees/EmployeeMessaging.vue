  ...
  <div class="window-container">
    <div class="chat-forms">
      <form @submit.prevent="createRoom" v-if="addNewRoom">
        <input type="text" placeholder="Add username to create a room" v-model="addRoomUsername" />
      ...
      <form @submit.prevent="addRoomUser" v-if="inviteRoomId">
        <input type="text" placeholder="Add user to the room" v-model="invitedUsername" />
...
// 	firebase
// } from '@/firestore'
import { parseTimestamp, isSameDay } from './dates'
...
export default {
  ...
  methods: {
    ...
    async fetchRooms() {
      this.resetRooms()
      //   const rooms = await roomsRef.where('users', 'array-contains', this.currentUserId).get()
      const rooms = [
        {
          roomId: 1,
          roomName: 'Room 1',
      ...
      Object.keys(roomList).forEach(key => {
        ...
        const roomAvatar =
          roomContacts.length === 1 && roomContacts[0].avatar ? roomContacts[0].avatar : require('@/assets/logo.png')
        formattedRooms.push({
          ...{
            roomId: key,
            avatar: roomAvatar,
