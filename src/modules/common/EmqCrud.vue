<template>
  <div class="emq-crud">
    ...
    <div
      v-if="
        (tableActions.includes('deleteAll') && this.selectedRecords.length !== 0) ||
    ...
    >
      ...
      <el-button
        ...
        @click="$refs.importExcelComp.showDialog()"
        >{{ $t('oper.imports') }}
      </el-button>
      <!-- Export -->
      <emq-export-excel
    ...
    <el-card class="table-wrapper">
      ...
      >
        <el-table-column v-if="tableActions.includes('deleteAll') || selection" type="selection" width="40">
        </el-table-column>
        <!-- Table other columns -->
        <slot name="tableColumns"></slot>
        <!-- Table operation -->
        <el-table-column v-if="has(`PUT,${url}/:id`) || has(`DELETE,${url}`)" min-width="90px" class-name="oper">
    ...
    <emq-import-excel v-if="tableActions.includes('import')" ref="importExcelComp" :url="url" :reloadData="loadRecords">
    </emq-import-excel>
...
import EmqSearchForm from '@/modules/common/EmqSearchForm'
  ...
  methods: {
    ...
    loadRecords(
      ...
      if (searchKeywordName && validSearchValue) {
        if (typeof searchKeywordValue === 'number') {
          // 数字精确检索
          params[searchKeywordName] = searchKeywordValue
        } else if (typeof searchKeywordValue === 'string') {
          // 字符串模糊检索
      ...
      // Time search
      if (searchTimeName && searchTimeValue && searchTimeValue.length > 0 && searchTimeValue[0] && searchTimeValue[1]) {
      ...
      httpGet(this.url, { params, ...this.httpConfig })
        .then(response => {
          ...
          if (this.isDefaultExpand) {
            // Expands the first row when there is a default expanded row
    ...
    loadLatestData(disableLoading, compares) {
      ...
      const findDiffData = (data, diffValues) => {
        ...
        if (valuesKey.length > 0) {
          ...
          res = unique(res.map(item => JSON.stringify(item))).map(item => JSON.parse(item))
        }
        return res
      }
    ...
    search(searchKeywordName, searchKeywordValue, tableFilterConditions, searchTimeName, searchTimeValue, config) {
    ...
    deleteRecords() {
      ...
      httpDelete(`${this.url.split('?')[0]}?ids=${this.willDelectIds}`)
        .then(response => {
