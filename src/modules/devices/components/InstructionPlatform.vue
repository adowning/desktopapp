<template>
  <div class="details-view instruction-platform-view">
    ...
    >
      <el-table-column prop="label" width="140px" :label="$t('products.dataPointName')"> </el-table-column>
      <el-table-column :label="$t('devices.setValue')">
        <template v-slot="scope">
          <div v-if="scope.row.attr.pointDataType === $variable.pointDataType.BOOLEAN">
            <el-radio v-model="scope.row.payload" :label="JSON.stringify(true)">true </el-radio>
            <el-radio v-model="scope.row.payload" :label="JSON.stringify(false)">false </el-radio>
          </div>
          <div v-else>
            <el-input
    ...
    <timer-publish-form ref="timerPublishForm" v-if="instructionType === 1"> </timer-publish-form>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      streamPointFormRules: {
        dataStream: [{ required: true, message: this.$t('products.dataStreamRequired') }],
      },
      dataPointRecords: [],
      url: '',
    }
  ...
  methods: {
    ...
    loadDataPoints(streamIntID) {
      ...
      httpGet(
        `/select_options/data_points?productID=${this.currentDevice.productID}&dataStreamIntID=${streamIntID}`,
      ).then(res => {
        ...
        this.dataPointRecords.forEach(record => {
          const dataPoint = pointDataType.find(item => item.value === record.attr.pointDataType)
    ...
    save() {
      this.$refs.streamPointForm.validate(valid => {
        ...
        if (this.instructionType === 0) {
          ...
          this.$refs.timerPublishForm.getTimerDate().then(res => {
