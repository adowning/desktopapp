...
export default {
  ...
  created() {
    const currentDevice = this.currentDevices.find(item => item.deviceIntID === parseInt(this.deviceIntID, 10))
    this.productIntID = currentDevice.productIntID
  },

  beforeDestroy() {
    const urls = this.$route.path
    if (!/\/devices\/[0-9]+/gi.test(urls)) {
      // Regular match url, clear cache when leaving device detail page
