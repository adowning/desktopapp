<template>
  ...
  <div class="client-table-view">
    ...
    >
      <!-- Custom button -->
      <template v-if="true" slot="customButton">
        <el-dropdown trigger="click" @command="handleCommand">
          ...
          <el-dropdown-menu class="device-custom-dropdown" slot="dropdown">
            ...
            <el-dropdown-item v-if="true" command="import" :disabled="$refs.rows.selectedRecords.length !== 0">
            ...
            <el-dropdown-item v-if="true" command="deviceControl" :disabled="$refs.rows.selectedRecords.length !== 1">
              <i class="iconfont icon-emq-command"></i>
              <span>{{ $t('devices.instruct') }}</span>
            </el-dropdown-item>
            <el-dropdown-item v-if="true" command="deviceTask" :disabled="$refs.rows.selectedRecords.length !== 1">
              <i class="iconfont icon-emq-task"></i>
              <span>{{ $t('devices.deviceTask') }}</span>
            </el-dropdown-item>
            <el-dropdown-item v-if="true" command="createGroup" :disabled="$refs.rows.selectedRecords.length === 0">
      ...
      <emq-button v-if="true" slot="createButton" class="create-btn" @click="createClient">
        + {{ $t('oper.createBtn') }}
      </emq-button>
      ...
      <template slot="tableColumns">
        ...
        <el-table-column v-if="true" prop="blocked" :label="$t('devices.blocked')">
          <template v-slot="scope">
      ...
      <template v-slot:customOper="props">
        <i v-if="true" class="oper-button iconfont icon-emq-edit" @click="showDetails(props.row, 'edit')"> </i>
      </template>
    </emq-crud>

    <!-- More operations depend on -->
    <emq-export-excel v-show="false" ref="exportButton" :url="url"> </emq-export-excel>
    <emq-import-excel v-if="true" ref="importExcelComp" :url="url" :reloadData="$refs.rows.loadRecords">
    </emq-import-excel>
