  ...
  <div class="client-table-view">
    ...
    >
      ...
      <template slot="tableColumns">
        ...
        <el-table-column v-else min-width="160" class="word-limit" prop="deviceName" :label="$t('devices.deviceName')">
          <template v-slot="scope">
            <a href="javascript:;" @click="showDetails(scope.row, 'view')">
              {{ scope.row.deviceName }}
            </a>
        ...
        <el-table-column prop="authTypeLabel" label="Employees">
          <template v-slot="props">
            {{ props.row.employeeList || '-' }}
          </template>
        </el-table-column>
        <el-table-column min-width="150" class="word-limit" prop="productName" :label="$t('devices.productName')">
        </el-table-column>
        ...
        <el-table-column v-else prop="cloudProtocolLabel" :label="$t('products.cloudProtocol')"> </el-table-column>
        ...
        <el-table-column v-if="has(`PUT,${url}/:id`)" prop="blocked" :label="$t('devices.blocked')">
          <template v-slot="scope">
        ...
        <el-table-column prop="lastConnection" min-width="150" sortable="custom" :label="$t('devices.lastConnection')">
        </el-table-column>
      </template>
    ...
    <emq-export-excel v-show="false" ref="exportButton" :url="url"> </emq-export-excel>
    <emq-import-excel v-if="customButtonVisible" ref="importExcelComp" :url="url" :reloadData="$refs.rows.loadRecords">
    </emq-import-excel>
    ...
    >
      <el-form ref="record" :model="record" :rules="groupRules">
        <el-form-item prop="groupName" :label="$t('groups.groupName')">
          <el-input v-model="record.groupName" :placeholder="$t('groups.groupNameRequired')"> </el-input>
        </el-form-item>
        <el-form-item prop="description" :label="$t('groups.description')">
          <el-input v-model="record.description" :placeholder="$t('groups.descriptionRequired')"> </el-input>
        </el-form-item>
      </el-form>
    </emq-dialog>
  </div>
...
import TabsCardHead from '@/modules/common/TabsCardHead'
import EmqCrud from '@/modules/common/EmqCrud'
import EmqButton from '@/modules/common/EmqButton'
import EmqExportExcel from '@/modules/common/EmqExportExcel'
import EmqImportExcel from '@/modules/common/EmqImportExcel'
import { deviceMixin } from '@/mixins/common'
import EmqDialog from '@/modules/common/EmqDialog'
...
export default {
  ...
  // {"/actions":["GET","POST","DELETE"],"/actions/:id":["PUT"],"/app_roles":["POST","DELETE","GET"],"/app_roles/:id":["PUT"],"/applications":["GET","POST","DELETE"],"/applications/:id":["PUT"],"/certs":["GET","POST","DELETE"],"/certs/:id":["PUT"],"/codec":["GET","POST","DELETE"],"/codec/:id":["PUT"],"/current_alerts":["GET","DELETE"],"/data_points":["GET","POST","DELETE"],"/data_points/:id":["PUT"],"/data_streams":["GET","POST","DELETE"],"/data_streams/:id":["PUT"],"/device_capability_data":["GET"],"/device_publish":["POST"],"/devices":["GET","POST","DELETE"],"/devices/:id":["PUT"],"/devices_export":["GET"],"/devices_import":["POST"],"/groups":["GET","POST","DELETE"],"/groups/:id":["PUT"],"/history_alerts":["GET","DELETE"],"/invitations":["POST","DELETE","GET"],"/login_logs":["GET"],"/messages":["GET","DELETE"],"/products":["GET","POST","DELETE"],"/products/:id":["PUT"],"/roles":["POST","DELETE","GET"],"/roles/:id":["PUT"],"/rules":["GET","POST","DELETE","GET","POST","DELETE"],"/rules/:id":["PUT","PUT"],"/run_code":["POST"],"/timer_publish":["GET","POST","DELETE"],"/users":["POST","DELETE","GET"],"/users/:id":["PUT"]}
  // {"/actions":["GET"],"/app_roles":["GET"],"/applications":["GET"],"/certs":["GET"],"/codec":["GET"],"/codec/:id":["PUT"],"/current_alerts":["GET"],"/data_points":["GET"],"/data_streams":["GET"],"/device_capability_data":["GET"],"/devices":["GET"],"/devices_export":["GET"],"/groups":["GET"],"/history_alerts":["GET"],"/invitations":["GET"],"/login_logs":["GET"],"/logo_info":["GET","PUT"],"/messages":["GET"],"/products":["GET"],"/roles":["GET"],"/rules":["GET","GET"],"/system_info":["GET","PUT"],"/tenants":["GET"],"/tenants/:id":["PUT"],"/timer_publish":["GET"],"/users":["GET"]}
  // {"token":"eyJhbGciOiJIUzUxMiIsImlhdCI6MTU4MzY5NzU3NywiZXhwIjoxNTg2Mjg5NTc3fQ.eyJ1c2VyX2lkIjoyNjIsInJvbGVfaWQiOjIsInRlbmFudF91aWQiOiJDTktxWGZhblkifQ.7PglSakdVSE8DzXb1RsUzzTL2fz57gimGXBpH5NuFxM7ISPlTE8x2nERCvySVgQl8SalhX-x_5KhEPnwNQ_0zw","userIntID":262,"username":"ash","tenantType":2,"tenantID":"CNKqXfanY"}
  // {"token":"eyJhbGciOiJIUzUxMiIsImlhdCI6MTU4Mzg3MTE3MywiZXhwIjoxNTg0MTMwMzczfQ.eyJ1c2VyX2lkIjoxLCJyb2xlX2lkIjoxLCJ0ZW5hbnRfdWlkIjpudWxsfQ.9sjytGop0UPMDHG_N0D_gkEGYi4u-HGnIPiaWs51Qd0wSgIhU0k4cl8lKXZvAlRdrq4etaGtNnAIVczAnLWo-w","userIntID":1,"username":"admin","tenantType":0,"tenantID":null}
  ...
  data() {
    return {
      ...
      record: {},
      groupRules: {
        groupName: [{ required: true, message: this.$t('groups.groupNameRequired'), trigger: 'blur' }],
        description: [{ required: true, message: this.$t('groups.descriptionRequired'), trigger: 'blur' }],
      },
    }
  },
  ...
  methods: {
    ...
    localCache(cache) {
      ...
      const hasExist = this.currentDevices.find(item => item.deviceIntID === cache.id)
