  ...
  <div class="clients-charts-view">
    ...
    <div v-if="!noData" class="chart-main" v-loading="chartLoading" :element-loading-text="$t('oper.isLoading')">
      <div v-for="(item, index) in chartsData" :key="index">
        <el-card v-if="displayCharts.includes(item.chartID)" class="chart-main__card">
          <div slot="header">
            <span>{{ item.chartName }}</span>
          </div>
          <div class="grid-content">
            <!-- Line chart data points -->
            <line-chart :chartId="`device-data-chart-${index}`" :yTitle="item.chartName" :chartData="item.chartData">
            </line-chart>
          </div>
        </el-card>
      </div>
    ...
    <emq-dialog :title="$t('charts.customChart')" :visible.sync="dialogVisible" @confirm="confirmCustomChart">
      <el-popover ref="addDevicePopover" placement="top-start" trigger="hover" :content="$t('charts.showDataPoints')">
      </el-popover>
      <i
      ...
      <el-checkbox-group v-model="chartsID">
        <el-checkbox v-for="(item, index) in chartsData" class="data-point__tag" :key="index" :label="item.chartID">
          {{ item.chartName }}
        </el-checkbox>
      </el-checkbox-group>
    </emq-dialog>
...
import LineChart from '@/modules/common/charts/SingleLine'
