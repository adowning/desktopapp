<template>
  ...
  >
    <el-radio-group class="command-type" v-model="commandType">
      ...
      <el-radio v-if="showPlatForm" :label="1"> {{ $t('devices.platformInstruct') }}</el-radio>
      <el-radio v-if="showLwM2M" :label="2">{{ $t('devices.lwm2mInstruct') }}</el-radio>
    </el-radio-group>
  ...
  computed: {
    showPlatForm() {
      if (this.currentDevice.deviceID) {
        ...
        return (
          cloudProtocol === this.$variable.cloudProtocol.MQTT || cloudProtocol === this.$variable.cloudProtocol.LWM2M
        )
      }
      return false
    },
