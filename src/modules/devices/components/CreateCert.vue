  ...
  <div class="create-cert-view">
    ...
    >
      ...
      <el-row v-if="!isCreated" :gutter="20">
        <el-form ref="record" label-position="top" :model="record" :rules="formRules">
          <el-col :span="12">
      ...
      <div v-if="isCreated" :gutter="20" class="cert-created">
        ...
        <div>
          <el-row v-loading="loading" v-if="isCreated" class="create-table">
            ...
            <el-col :span="4">
              <div class="cert-download">
                <a href="javaScript:;" @click="downloadFile(download.cert.fileName, download.cert.content)">
                  {{ $t('certs.download') }}
                </a>
              </div>
            </el-col>
            ...
            <el-col :span="4">
              <div class="cert-download">
                <a href="javaScript:;" @click="downloadFile(download.key.fileName, download.key.content)">
                  {{ $t('certs.download') }}
                </a>
              </div>
            </el-col>
            ...
            <el-col :span="4">
              <div class="cert-download">
                <a @click="downloadFile(download.root.fileName, download.root.content)" href="javaScript:;">
                  {{ $t('certs.download') }}
                </a>
              </div>
            </el-col>
      ...
      <div v-if="!isCreated" slot="footer" class="dialog-footer">
        <el-button type="text" size="small" @click="visible = false">
          {{ $t('oper.cancel') }}
        </el-button>
...
export default {
  ...
  data() {
    return {
      ...
      download: {},
      formRules: {
        certName: [{ required: true, message: this.$t('certs.nameRequired') }],
        enable: [{ required: true, message: this.$t('certs.enableRequired') }],
      },
    }
  },
