<template>
  <div class="certs-view">
    <!-- Create Certs -->
    <create-cert :dialogVisible.sync="createCertDialogVisible" :callbackFunction="updateTableData"></create-cert>
      ...
      <template slot="tableColumns">
        <el-table-column prop="certName" :label="$t('certs.name')">
          <template v-slot="scope">
            <router-link :to="{ path: `/security/certs/${scope.row.id}`, query: { oper: 'view' } }">
              {{ scope.row.certName }}
            </router-link>
          </template>
        </el-table-column>
        ...
        <el-table-column v-else :label="$t('certs.enable')" prop="enable">
          <template v-slot="scope">
            {{ scope.row.enable === 1 ? $t('certs.isTrue') : $t('certs.isFalse') }}
          </template>
        </el-table-column>
        <el-table-column prop="createAt" :label="$t('certs.createAt')" sortable="custom"> </el-table-column>
      </template>
    </emq-crud>
  </div>
</template>
