<template>
  <div class="details-view device-details-control-view">
    ...
    <el-row class="custom-header">
      ...
      <el-col :span="8">
        <emq-button icon="create" @click="addVisible = true">
          {{ $t('oper.createBtn') }}
        </emq-button>
      </el-col>
    </el-row>
    ...
    >
      <template slot="tableColumns">
        <el-table-column width="160px" prop="publishStatusLabel" :label="$t('devices.publishStatusLabel')">
        </el-table-column>
        <el-table-column prop="payload" min-width="180px" :label="$t('devices.publishStatusContent')">
        </el-table-column>
        ...
        <el-table-column v-else width="140px" prop="topic" :label="$t('devices.publishTopic')"> </el-table-column>

        <el-table-column min-width="60px" prop="msgTime" :label="$t('devices.publishCreateAt')"> </el-table-column>
      </template>
    </emq-crud>
    ...
    >
      <template slot="tableColumns">
        <el-table-column prop="taskName" :label="$t('devices.taskName')"> </el-table-column>
        <el-table-column prop="taskStatusLabel" :label="$t('devices.publishStatusLabel')"> </el-table-column>
        <el-table-column min-width="110px" :label="$t('devices.publishCreateAt')">
          <template v-slot="{ row }">
            {{ timeFormat(row) }}
          </template>
        </el-table-column>
        <el-table-column min-width="260px" prop="payload" :label="$t('devices.publishStatusContent')">
          <template v-slot="{ row }">
            {{ row.payload }}
          </template>
        </el-table-column>
        <el-table-column prop="timerTypeLabel" :label="$t('devices.timingType')"> </el-table-column>
      </template>
    </emq-crud>
  ...
  methods: {
    ...
    timeFormat(timer) {
      ...
      if (timer.intervalTime.minute || timer.intervalTime.minute === 0) {
        minute = timer.intervalTime.minute <= 9 ? `0${timer.intervalTime.minute}` : timer.intervalTime.minute
      }
      if (weekTitle) {
